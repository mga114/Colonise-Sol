...
class MyGame(arcade.Window):  # class built into arcade where THE MAGIC HAPPENS
    ...
    def on_draw(self):
        ...
        self.ph = 0  # resets placeholder variable
        for resource in self.resource:  # loops through all resources
            arcade.draw_text('+' + str(resource[0]), 90, 585 - (90 * self.ph), arcade.color.GREEN, 15)  # draws the net gain of resources at the left side of screen
            arcade.draw_text('-' + str(resource[1]), 90, 550 - (90 * self.ph), arcade.color.RED, 15)  # draws the net requirement of resources at the left side of screen
            if resource[1] - resource[0] == 0:  # tests for what colour the text should be
                arcade.draw_text(str(resource[0] - resource[1]), 140, 560 - (90 * self.ph), arcade.color.YELLOW, 20)  # draws the net gain - loss of resources
            elif resource[1] - resource[0] > 0:  # tests for what colour the text should be
                arcade.draw_text(str(resource[0] - resource[1]), 130, 560 - (90 * self.ph), arcade.color.RED, 20)  # draws the net gain - loss of resources
            else:   # tests for what colour the text should be
                arcade.draw_text('+' + str(resource[0] - resource[1]), 130, 560 - (90 * self.ph), arcade.color.GREEN, 20)  # draws the net gain - loss of resources
            self.ph += 1  # changes the placeholder by one to change the position of the text being drawn
        i = 0  # resets the placeholder variable
        for planet in Planet.names:  # draws all of the text for the resources on each planet on the right hand of screen
            arcade.draw_text(str(planet.population[0] - planet.population[1]), 995, 600 - (92.5 * i), arcade.color.WHITE, 12)
            arcade.draw_text(str(planet.energy[0] - planet.energy[1]), 1050, 600 - (92.5 * i), arcade.color.WHITE, 12)
            arcade.draw_text(str(planet.water[0] - planet.water[1]), 995, 550 - (92.5 * i), arcade.color.WHITE, 12)
            arcade.draw_text(str(planet.minerals[0] - planet.minerals[1]), 1050, 550 - (92.5 * i), arcade.color.WHITE, 12)
            i += 1  # changes placeholder by 1 to change position
        if self.win is True:  # draws win and lose screen
            arcade.draw_texture_rectangle(540, 360, SCREEN_WIDTH, SCREEN_HEIGHT, arcade.load_texture('sprites/gui/win condition.png'))
        elif self.win is False:
            arcade.draw_texture_rectangle(540, 360, SCREEN_WIDTH, SCREEN_HEIGHT, arcade.load_texture('sprites/gui/lose condition.png'))
...
def draw_gui():  # draws most of the gui for the game
    ...
    i = -1  # resets placeholder variable
    for planets in Planet.name:  # loops through all planets
        i += 1
        ##arcade.draw_text(planets, 900, 690 - (90 * i), arcade.color.WHITE, 15, font_name='trench100free.ttf')  # displays name of planets on right hand side
    if Ship.landed is not None:  # tests if game state is landed
        ...
        resources = [Ship.landed.population, Ship.landed.minerals, Ship.landed.pollution, Ship.landed.water, Ship.landed.housing]  # gathers all resources in one place
        colour = ''  # resets placeholder variables
        i = 0
        for resource in resources:  # loops through all resources to print text at the bottom of the screen
            arcade.draw_text(str(resource[0]), 300 + (140 * i), 140, arcade.color.GREEN, 10)  # shows the gain for the resource
            arcade.draw_text(str(resource[1]), 300 + (140 * i), 100, arcade.color.RED, 10)  # shows the requirement for the resource
            if resource[0] - resource[1] == 0:  # tests if the net gain for the resource is positive, negative, or neutral
                colour = arcade.color.YELLOW  # sets colour accordingly
            elif resource[0] - resource[1] < 0:
                colour = arcade.color.RED
            else:
                colour = arcade.color.GREEN
            arcade.draw_text(str(resource[0] - resource[1]), 320 + (140 * i), 120, colour, 15)  # draws net gain/loss for resource
            i += 1  # changes position of text
        resources = [Ship.landed.energy, Ship.landed.food]  # does the same as above but different positions for energy and food
        i = 0
        colour = ''
        for resource in resources:
            arcade.draw_text(str(resource[0]), 300 + (560 * i), 60, arcade.color.GREEN, 10)
            arcade.draw_text(str(resource[1]), 300 + (560 * i), 20, arcade.color.RED, 10)
            if resource[0] - resource[1] == 0:
                colour = arcade.color.YELLOW
            elif resource[0] - resource[1] < 0:
                colour = arcade.color.RED
            else:
                colour = arcade.color.GREEN
            arcade.draw_text(str(resource[0] - resource[1]), 320 + (560 * i), 40, colour, 15)
            i += 1
